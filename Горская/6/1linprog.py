...
def simplex(c, A, b, tol=1e-9):
    ...
    """
    Аналогия для понимания процесса

Представьте, что у вас есть несколько видов ингредиентов (ресурсов) для приготовления блюда. Вы хотите определить, сколько блюда можно приготовить, чтобы заработать как можно больше денег, но ингредиенты ограничены.

    Исходное решение: Вы начинаете с простого рецепта, в котором не используются никакие специальные техники, просто берётся максимум «избыточных» ингредиентов.

    Итерации метода симплекс: Затем вы постепенно пробуете варьировать пропорции, чтобы увеличить вкус (доход). Вы смотрите, какие изменения (включение новой переменной – новый ингредиент) могут улучшить результат, и корректируете рецепт.

    Финальная рецептура: В итоге получаете рецепт, где некоторые ингредиенты полностью использованы (лимитирующие ресурсы), а в других ещё остаётся запас. Этот рецепт дает максимальную прибыль.
    """
...
if __name__ == "__main__":
    ...
    # -----------------------------
    products = ['Π₁', 'Π₂', 'Π₃', 'Π₄']
    income_coeff = np.array([1, 4, 3, 5], dtype=float)  # Доход от единицы продукции
